...
import { State, collectionsAdapter, initialState } from './collections.reducer';
import * as CollectionsSelectors from './collections.selectors';

describe('Collections Selectors', () => {
  const ERROR_MSG = 'No Error Available';
  const getCollectionsId = (it) => it['id'];
  ...
  describe('Collections Selectors', () => {
    ...
    it('getSelected() should return the selected Entity', () => {
      const result = CollectionsSelectors.getSelected(state);
